            ...
            if (result.status === 403 || result.status === 500) {
                return result.json().then( (errorMsg) => {
                    ...
                    if (result.status === 500) {
                        ...
                        const extractedString = errorMsg.error.substring(errorMsg.error.lastIndexOf("."), errorMsg.error.lastIndexOf("'"));
                        msg = extractedString.includes('PRIMARY') ? "Username already exists"
                        : extractedString.includes('email') ? extractedString.charAt(0).toUpperCase + extractedString.slice(1) + "already exists": "There was an error";
                    }
